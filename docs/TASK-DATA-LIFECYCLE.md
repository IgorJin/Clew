...
Все карточки имеют статус `planned`, размер S/M и `release: unassigned`. Текущий выбранный релиз — v0.10 Keyboard-first. В очереди контекста первым идёт scout CLEW-123–126; независимая storage ветка начинается с CLEW-105. Номер карточки не определяет порядок исполнения.
