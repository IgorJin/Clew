...
Storage-карточки CLEW-105–122 остаются `planned`; CLEW-123 и CLEW-124 завершены, а CLEW-125 готова следующей. Все они имеют размер S/M и `release: unassigned`. В очереди контекста первым идёт scout CLEW-123–126; независимая storage ветка начинается с CLEW-105. Номер карточки не определяет порядок исполнения.
