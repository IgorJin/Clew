...
The top bar also exposes a gear-icon Settings modal (CLEW-099). The modal is UI-only: its Agent chapter lets the operator pick one of Codex CLI, Claude CLI, or OpenCode CLI as a local on-device preference. The choice issues no API calls, renders identically with or without a daemon, and is explicitly marked as unverified — it does not change execution. Availability checks and backend wiring are deferred follow-ups.

The current keyboard surface contains `Cmd/Ctrl+K` for the project/task palette, arrow navigation inside the palette, Enter for selection, and Escape for dialogs and popovers. The proposed complete keyboard model, including numbered task switching, terminal/change actions, and key hints while Command is held, is documented in [Keyboard controls](./KEYBOARD-CONTROLS.md).

When the selected task has an active controller-local interactive worker, the UI automatically expands its agent card and attaches the embedded terminal. It does not automatically launch an external terminal application. External opening remains an explicit operator action; Runner-local terminals must be opened on the Runner host.
