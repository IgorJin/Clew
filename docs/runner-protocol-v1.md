...
Allowed transport data is limited to stable identities, capability names, logical workspace IDs, the bounded public Task contract needed for execution, a validated bounded Execution Brief, normalized lifecycle summaries, revision IDs, bounded plans/reviews/evidence, and aggregate usage. The Execution Brief contains only Task- and run-level facts; the Runner compiles the native harness prompt locally. Harness-generated or compiled prompts and hidden reasoning are never transported. Credentials, environment values, host paths, arbitrary file contents, raw native output, repository archives, and PTY bytes are forbidden. Terminal capability is projected as `runner_local_terminal`; terminal traffic remains on the Runner host.
