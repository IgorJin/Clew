...
- `CLEW_REVIEW_MODEL` — Codex reviewer model (defaults to `gpt-5.6-luna`);
...
When other role model variables are absent, Clew lets the installed Codex CLI select its authenticated-account default. Review uses `gpt-5.6-luna` by default and can be overridden through `models.reviewer` or `CLEW_REVIEW_MODEL`.
