...
| 14  | Optional telemetry preserves lifecycle behavior when disabled or unavailable | observability no-op/missing-runtime tests                                   |
| 15  | Telemetry correlation is persisted across task and run boundaries            | telemetry context migration and Store integration                           |
| 16  | Telemetry attributes remain metadata-only and secret-safe                    | allowlisted span contract and redaction boundary                            |
