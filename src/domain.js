export const TASK_STATES = Object.freeze([
  'DRAFT', 'PLAN_READY', 'QUEUED', 'EXECUTING', 'VERIFYING', 'REVIEWING',
  'WAITING_FOR_HUMAN', 'READY', 'COMPLETED', 'FAILED', 'CANCELLED', 'BLOCKED'
]);
...
const transitions = {
  ...
  COMPLETED: [], FAILED: ['QUEUED', 'CANCELLED'], CANCELLED: [], BLOCKED: ['QUEUED']
};
...
export function validateContract(input) {
  ...
  for (const field of ['id', 'title', 'goal', 'profile']) {
    if (typeof input[field] !== 'string' || !input[field].trim()) throw new Error(`task.${field} is required`);
  }
  if (!/^[A-Za-z0-9][A-Za-z0-9_.-]{1,63}$/.test(input.id)) throw new Error('task.id must contain 2-64 safe characters');
  ...
  for (let i = 0; i < acceptance.length; i++) {
    const item = typeof acceptance[i] === 'string' ? { id: `AC-${i + 1}`, criterion: acceptance[i] } : acceptance[i];
  ...
  if (!['quick', 'standard', 'deep'].includes(input.profile)) throw new Error('task.profile must be quick, standard, or deep');
  return { ...input, risk: input.risk ?? 'medium', base_ref: input.base_ref ?? 'HEAD', acceptance: acceptance.map((x, i) => typeof x === 'string' ? { id: `AC-${i + 1}`, criterion: x } : x) };
}

export function effectiveProfile(name) {
  const common = { maxAttempts: 3, verification: 'targeted' };
  if (name === 'quick') return { ...common, name, mode: 'direct', harness: 'fake', review: false, architecture: false, maxWorkers: 1 };
  if (name === 'standard') return { ...common, name, mode: 'isolated', harness: 'fake', review: true, architecture: false, maxWorkers: 1 };
  return { ...common, name, mode: 'parallel', harness: 'fake', review: true, architecture: true, integration: true, maxWorkers: 3 };
}
