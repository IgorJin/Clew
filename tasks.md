...
**Active target:** Scout v0 context experiment (`CLEW-126` complete; checkpoint/handoff work follows); detailed cards are tracked in [`tasks/`](./tasks/README.md)
...
| [CLEW-126](./tasks/done/CLEW-126.md) | S    | Scout pilot and context contract refinement (done)    | 125        |
...
| CLEW-043 | P1  | L    | Usage, token, and cost accounting                   | 042        | One idempotent record exists per native turn; a daily cron-driven pricing sync stores immutable provider/catalog snapshots; partial/unknown data stays explicit; total lifecycle cost across every attempt is reproducible in human/JSON output and the result manifest. |
...
| CLEW-077 | P0  | L    | Interactive worker response lifecycle           | 078, 079      | Completed Codex turns appear once in Task Thread and put the attached terminal into an explicit operator-waiting state without completing the Run.               |
| CLEW-078 | P0  | M    | Read-only Codex interactive turn monitor        | 068, 069, 072 | A stdio App Server reader observes the active TUI-owned thread, normalizes turn states, survives failures/restarts, and never becomes a second writer.           |
| CLEW-079 | P0  | M    | Worker responses and waiting state in Thread/UI | 078           | Durable completed-turn responses and live running/waiting attention state converge across event delivery, reconnect, reload, and explicit Finish worker.         |
